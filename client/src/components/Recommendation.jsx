...
function RecommendationsSection({ userId }) {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        toast.error("Error loading cities data");
      }
    };
  ...
  const getCityName = (cityId) => {
    ...
    const id = cityId?.$oid || cityId;
    const city = cities.find((c) => c._id === id || c._id?.$oid === id);
    return city ? city.City_Name : "Unknown City";
  };
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        const res = await axios.get(
          `${API_URL}/user/getRecommendations/${userId}`
        );
        setRecommendedJobs(res.data);
  ...
  const renderCard = (type, item) => {
    const baseStyle =
      "bg-[#F9F5F0] border border-[#e5d5c3] rounded-2xl p-5 hover:shadow-lg transition-all duration-300 flex flex-col justify-between break-words min-w-[300px] max-w-[350px]";

    const titleClass =
      "text-md font-semibold text-gray-800 mb-1 line-clamp-3 break-words whitespace-normal overflow-visible";
    const subtitleClass =
      "text-sm text-gray-600 mb-2 leading-snug break-words whitespace-normal";
    ...
    switch (type) {
      case "Courses":
        return (
          <div key={item._id} className={baseStyle}>
            <div className="flex items-start gap-4 mb-4 overflow-hidden">
              <div className="w-10 h-10 rounded-xl bg-[#f0e8dc] flex items-center justify-center flex-shrink-0">
                <Laptop className="text-[#8B4513]" size={20} />
              </div>
        ...
        return (
          <div key={item._id} className={baseStyle}>
            ...
            <div className="flex justify-between text-sm text-gray-700 mt-auto">
              <span className="flex items-center gap-1">
                <MapPin size={14} />
                {getCityName(item.Location?._id || item.Location)}
              </span>
              <span className="text-[#8B4513] font-semibold overflow-hidden">
    ...
    <div className="bg-white rounded-2xl shadow-lg p-6 w-full">
      ...
      <div className="overflow-x-auto flex gap-6 mb-6 border-b border-[#e5d5c3] pb-2">
        {tabs.map((tab) => (
          <button
            key={tab}
            className={`pb-1 text-base transition-all whitespace-nowrap ${
              activeTab === tab
                ? "border-b-2 border-[#8B4513] text-[#8B4513] font-semibold"
                : "text-gray-500 hover:text-[#8B4513]"
            }`}
      ...
      <div className="flex gap-6 overflow-x-auto pb-2">
        {currentData.length > 0 ? (
          ...
          <p className="text-gray-500 text-center w-full py-8">
            No {activeTab.toLowerCase()} recommendations available.
          </p>
        )}
      </div>
    </div>
